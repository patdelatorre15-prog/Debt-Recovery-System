const apiBase=window.DRS_CONFIG.apiBase.replace(/\/$/,'');
...
const entitlementForm=document.querySelector('#entitlementForm');
const entitlementPlan=entitlementForm.elements.plan;
const entitlementStart=entitlementForm.elements.startsOn;
const entitlementEnd=entitlementForm.elements.endsOn;
function addPlanDuration(date,plan){
  ...
  const months=Number(String(plan).replace('months',''));
...
function updateEntitlementEnd(){entitlementEnd.value=addPlanDuration(entitlementStart.value,entitlementPlan.value);}
entitlementEnd.readOnly=true;
entitlementPlan.addEventListener('change',updateEntitlementEnd);
entitlementStart.addEventListener('input',updateEntitlementEnd);
entitlementStart.addEventListener('change',updateEntitlementEnd);
...
entitlementForm.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget;try{updateEntitlementEnd();const body=Object.fromEntries(new FormData(form));const result=await api('/api/admin/entitlements',{method:'POST',headers:{'idempotency-key':crypto.randomUUID()},body:JSON.stringify(body)});show(result.status==='active'?`Access is active. Entitlement ${result.id} was recorded.`:'The access grant is saved and will activate when this Gmail first signs in.');form.reset();updateEntitlementEnd();}catch(error){show(error.message,true);}});
...
api('/api/session').then(({user})=>{if(user.role!=='admin'){location.replace('index.html');return null;}document.body.classList.remove('admin-pending');return Promise.all([loadAttention(),loadAudit()]);}).catch(error=>{location.replace('index.html');});
