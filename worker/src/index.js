...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (route === 'POST /api/goals/delete') return cors(await deleteGoal(request,env,user),request,env);
      ...
      if (route === 'POST /api/living-budgets/bulk') return cors(await saveLivingBudgetsBulk(request,env,user),request,env);
      if (route === 'POST /api/living-bill-plans/bulk') return cors(await saveLivingBillPlansBulk(request,env,user),request,env);
      if (route === 'POST /api/living-bills') return cors(await saveLivingBill(request,env,user),request,env);
      if (route === 'POST /api/living-bills/pay') return cors(await saveAndPayLivingBill(request,env,user),request,env);
      if (route === 'POST /api/expenses') return cors(await recordExpense(request,env,user),request,env);
      if (route === 'POST /api/ledger/reverse') return cors(await reverseLedgerEntry(request,env,user),request,env);
...
async function listActivity(request,env,user){
  ...
  const fields=`l.*,CASE WHEN reversed.original_entry_id IS NULL THEN 0 ELSE 1 END reversed,CASE WHEN created.reversal_entry_id IS NULL THEN 0 ELSE 1 END is_reversal,reversed.note reversal_note`;
  const joins=`FROM ledger_entries l LEFT JOIN ledger_reversals reversed ON reversed.user_id=l.user_id AND reversed.original_entry_id=l.id LEFT JOIN ledger_reversals created ON created.user_id=l.user_id AND created.reversal_entry_id=l.id`;
  const statement=category
    ?env.DB.prepare(`SELECT ${fields} ${joins} WHERE l.user_id=? AND l.category=? ORDER BY l.occurred_on DESC,l.created_at DESC LIMIT ?`).bind(user.id,category,limit)
    :env.DB.prepare(`SELECT ${fields} ${joins} WHERE l.user_id=? ORDER BY l.occurred_on DESC,l.created_at DESC LIMIT ?`).bind(user.id,limit);
...
async function listGoals(request,env,user){
  ...
  const achievements=achievementTable?await env.DB.prepare(`SELECT a.goal_id,a.achieved_amount_minor,a.achieved_on,g.category,g.name goal_name,g.goal_type,g.target_amount_minor,g.created_at goal_created_at,COALESCE(-SUM(CASE WHEN l.entry_type IN ('goal_allocation','goal_use') THEN l.amount_minor ELSE 0 END),0) saved_minor FROM goal_achievements a JOIN goals g ON g.id=a.goal_id LEFT JOIN ledger_entries l ON l.related_type='goal' AND l.related_id=g.id WHERE a.user_id=? ${category?'AND g.category=?':''} GROUP BY a.id ORDER BY a.achieved_on DESC,a.created_at DESC`).bind(...(category?[user.id,category]:[user.id])).all():{results:[]};
...
async function deleteGoal(request,env,user){
  const b=await readJson(request),goal=await ownedGoal(env,user,b.goalId),occurred=validDate(b.date)||dateOnly(new Date());
  if(!goal)return reply({error:'invalid_goal'},400);
  const saved=await goalBalance(env,user,goal.id),now=new Date().toISOString(),statements=[];
  if(saved>0)statements.push(env.DB.prepare(`INSERT INTO ledger_entries(id,user_id,occurred_on,entry_type,category,amount_minor,related_type,related_id,description,created_at,idempotency_key) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),user.id,occurred,'goal_use',goal.category,saved,'goal',goal.id,`Released funds from deleted goal · ${goal.name}`,now,idempotency(request)));
  statements.push(env.DB.prepare(`UPDATE goals SET status='closed',updated_at=? WHERE id=? AND user_id=? AND status='active'`).bind(now,goal.id,user.id));
  await env.DB.batch(statements);return reply({goalId:goal.id,releasedMinor:saved},200);
}
...
async function saveLivingBudgetsBulk(request,env,user){
  const b=await readJson(request),rows=Array.isArray(b.rows)?b.rows:[];
  if(!rows.length||rows.length>100)return reply({error:'invalid_living_budget_table'},400);
  const names=new Set(),normalized=[];
  for(const row of rows){const name=clean(row.name,120),amount=moneyMinor(row.amount??row.plan),active=row.active!==false&&row.active!==0;if(!name||amount<=0||names.has(name.toLowerCase()))return reply({error:names.has(name.toLowerCase())?'duplicate_living_plan_name':'invalid_living_budget_table'},400);names.add(name.toLowerCase());normalized.push({id:String(row.id||crypto.randomUUID()),name,amount,active});}
  const now=new Date().toISOString(),statements=normalized.map(x=>env.DB.prepare(`INSERT INTO living_plans(id,user_id,name,plan_type,planned_amount_minor,due_day,active,effective_from,frequency,due_day_secondary,created_at,updated_at) VALUES(?,?,?,'budget',?,NULL,?,?, 'monthly',NULL,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,planned_amount_minor=excluded.planned_amount_minor,active=excluded.active,updated_at=excluded.updated_at WHERE user_id=excluded.user_id AND plan_type='budget'`).bind(x.id,user.id,x.name,x.amount,x.active?1:0,dateOnly(new Date()),now,now));
  await env.DB.batch(statements);return reply({saved:normalized.length},201);
}
async function saveLivingBillPlansBulk(request,env,user){
  const b=await readJson(request),rows=Array.isArray(b.rows)?b.rows:[];
  if(!rows.length||rows.length>100)return reply({error:'invalid_living_bill_plan_table'},400);
  const names=new Set(),normalized=[];
  for(const row of rows){const name=clean(row.biller??row.name,120),amount=moneyMinor(row.amount??row.plan),due=Number(row.dueDay),frequency=['monthly','twice_monthly','weekly','daily'].includes(row.frequency)?row.frequency:null,second=frequency==='twice_monthly'?Number(row.dueDaySecondary):null,active=row.active!==false&&row.active!==0;if(!name||amount<=0||!Number.isInteger(due)||due<1||due>31||!frequency||(frequency==='twice_monthly'&&(!Number.isInteger(second)||second<1||second>31||second===due))||names.has(name.toLowerCase()))return reply({error:names.has(name.toLowerCase())?'duplicate_living_plan_name':'invalid_living_bill_plan_table'},400);names.add(name.toLowerCase());normalized.push({id:String(row.id||crypto.randomUUID()),name,amount,due,second,frequency,active});}
  const now=new Date().toISOString(),statements=normalized.map(x=>env.DB.prepare(`INSERT INTO living_plans(id,user_id,name,plan_type,planned_amount_minor,due_day,active,effective_from,frequency,due_day_secondary,created_at,updated_at) VALUES(?,?,?,'bill',?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,planned_amount_minor=excluded.planned_amount_minor,due_day=excluded.due_day,active=excluded.active,frequency=excluded.frequency,due_day_secondary=excluded.due_day_secondary,updated_at=excluded.updated_at WHERE user_id=excluded.user_id AND plan_type='bill'`).bind(x.id,user.id,x.name,x.amount,x.due,x.active?1:0,dateOnly(new Date()),x.frequency,x.second,now,now));
  await env.DB.batch(statements);return reply({saved:normalized.length},201);
}
...
async function saveAndPayLivingBill(request,env,user){
  const b=await readJson(request),occurred=validDate(b.paymentDate),actual=moneyMinor(b.actualAmount),payment=moneyMinor(b.paidAmount),settles=Boolean(b.settlesFull),plan=await env.DB.prepare(`SELECT * FROM living_plans WHERE id=? AND user_id=? AND plan_type='bill' AND active=1`).bind(String(b.planId||''),user.id).first();
  if(!plan||!occurred||actual<0||payment<0)return reply({error:'invalid_bill_payment'},400);
  if(payment>0&&payment!==Number(plan.planned_amount_minor)&&!settles)return reply({error:'bill_difference_confirmation_required'},409);
  if(payment>0){const available=await categoryBalance(env,user,'living');if(available<payment)return reply({error:'insufficient_category_funds',availableMinor:available},409);}
  const month=occurred.slice(0,7),due=billDueDate(month,Number(plan.due_day)),now=new Date().toISOString();
  let bill=await env.DB.prepare(`SELECT * FROM living_bill_instances WHERE plan_id=? AND billing_month=?`).bind(plan.id,month).first();
  const billId=bill?.id||crypto.randomUUID(),previousPaid=Number(bill?.paid_amount_minor||0),totalPayment=previousPaid+payment,trackedPaid=Math.min(totalPayment,actual),status=settles||totalPayment>=actual?'paid':totalPayment>0?'partially_paid':'unpaid',statements=[];
  if(bill)statements.push(env.DB.prepare(`UPDATE living_bill_instances SET due_on=?,actual_amount_minor=?,paid_amount_minor=?,status=?,updated_at=? WHERE id=? AND user_id=?`).bind(due,actual,trackedPaid,status,now,bill.id,user.id));
  else statements.push(env.DB.prepare(`INSERT INTO living_bill_instances(id,user_id,plan_id,billing_month,due_on,actual_amount_minor,paid_amount_minor,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)`).bind(billId,user.id,plan.id,month,due,actual,trackedPaid,status,now,now));
  let ledgerId=null;
  if(payment>0){ledgerId=crypto.randomUUID();statements.push(env.DB.prepare(`INSERT INTO ledger_entries(id,user_id,occurred_on,entry_type,category,amount_minor,related_type,related_id,description,metadata_json,created_at,idempotency_key) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`).bind(ledgerId,user.id,occurred,'bill_payment','living',-payment,'living_plan',plan.id,`Bill payment · ${plan.name}`,JSON.stringify({billInstanceId:billId,plannedAmountMinor:Number(plan.planned_amount_minor),actualAmountMinor:actual,settlesFull:settles}),now,idempotency(request)));statements.push(env.DB.prepare(`INSERT INTO living_bill_payment_operations(id,bill_instance_id,expected_paid_minor,payment_amount_minor,ledger_entry_id,created_at) VALUES(?,?,?,?,?,?)`).bind(crypto.randomUUID(),billId,previousPaid,payment,ledgerId,now));}
  await env.DB.batch(statements);return reply({billInstanceId:billId,ledgerEntryId:ledgerId,actualAmountMinor:actual,paidAmountMinor:trackedPaid,status},201);
}
...
async function reverseLedgerEntry(request,env,user){
  const b=await readJson(request),entryId=String(b.entryId||''),note=clean(b.note,500),occurred=validDate(b.date)||dateOnly(new Date()),key=idempotency(request),now=new Date().toISOString();
  if(!entryId||!note)return reply({error:'reversal_note_required'},400);
  const original=await env.DB.prepare(`SELECT * FROM ledger_entries WHERE id=? AND user_id=?`).bind(entryId,user.id).first();
  if(!original)return reply({error:'ledger_entry_not_found'},404);
  if(original.entry_type==='reversal')return reply({error:'reversal_not_allowed'},409);
  const allowed=['allocation','direct_funds','expense','bill_payment','transfer_in','transfer_out'];if(!allowed.includes(original.entry_type))return reply({error:'reversal_not_allowed'},409);
  let rows=[original];
  if(['transfer_in','transfer_out'].includes(original.entry_type))rows=(await env.DB.prepare(`SELECT * FROM ledger_entries WHERE user_id=? AND related_type='transfer' AND related_id=?`).bind(user.id,original.related_id).all()).results;
  if(original.entry_type==='allocation'){const source=original.source_entry_id;rows=(await env.DB.prepare(`SELECT * FROM ledger_entries WHERE user_id=? AND (id=? OR source_entry_id=?)`).bind(user.id,source,source).all()).results;}
  const ids=rows.map(x=>x.id);for(const id of ids){if(await env.DB.prepare(`SELECT id FROM ledger_reversals WHERE user_id=? AND original_entry_id=?`).bind(user.id,id).first())return reply({error:'ledger_entry_already_reversed'},409);}
  const statements=[];let count=0;
  for(const row of rows){const reversalId=crypto.randomUUID();statements.push(env.DB.prepare(`INSERT INTO ledger_entries(id,user_id,occurred_on,entry_type,category,amount_minor,source_entry_id,related_type,related_id,description,metadata_json,created_at,idempotency_key) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(reversalId,user.id,occurred,'reversal',row.category,-Number(row.amount_minor),row.id,row.related_type,row.related_id,`Reversal · ${row.description}`,JSON.stringify({note,originalEntryId:row.id,originalEntryType:row.entry_type}),now,`${key}:${count++}`));statements.push(env.DB.prepare(`INSERT INTO ledger_reversals(id,user_id,original_entry_id,reversal_entry_id,note,created_at) VALUES(?,?,?,?,?,?)`).bind(crypto.randomUUID(),user.id,row.id,reversalId,note,now));
    if(row.entry_type==='bill_payment'){const operation=await env.DB.prepare(`SELECT o.*,b.actual_amount_minor,b.paid_amount_minor FROM living_bill_payment_operations o JOIN living_bill_instances b ON b.id=o.bill_instance_id WHERE o.ledger_entry_id=?`).bind(row.id).first();if(operation){const paid=Math.max(0,Number(operation.paid_amount_minor)-Number(operation.payment_amount_minor)),status=paid<=0?'unpaid':paid>=Number(operation.actual_amount_minor)?'paid':'partially_paid';statements.push(env.DB.prepare(`UPDATE living_bill_instances SET paid_amount_minor=?,status=?,updated_at=? WHERE id=? AND user_id=?`).bind(paid,status,now,operation.bill_instance_id,user.id));}}
  }
  await env.DB.batch(statements);return reply({reversedEntryId:entryId,reversedCount:rows.length,note},201);
}

async function listDebts(env,user){
  await reconcileOriginalBalanceInterest(env,user.id);
  await postDueInterest(env,new Date().toISOString(),500,user.id);
  const result=await env.DB.prepare(`SELECT d.*,c.name creditor_name,(SELECT json_object('effectiveOn',v.effective_on,'paymentAmountMinor',v.payment_amount_minor,'dueDate',v.due_date,'dueDateSecondary',v.due_date_secondary,'paymentFrequency',v.payment_frequency,'interestAnchorDate',COALESCE(v.interest_anchor_date,v.due_date),'interestMode',v.interest_mode,'interestValue',v.interest_value,'interestFrequency',v.interest_frequency,'interestBasis',v.interest_basis,'paymentPaused',v.payment_paused,'createdAt',v.created_at) FROM debt_agreement_versions v WHERE v.debt_id=d.id ORDER BY v.created_at DESC,v.effective_on DESC LIMIT 1) agreement FROM debts d JOIN creditors c ON c.id=d.creditor_id WHERE d.user_id=? ORDER BY CASE d.status WHEN 'active' THEN 0 WHEN 'paused' THEN 1 WHEN 'paid' THEN 2 WHEN 'archived' THEN 3 ELSE 4 END,c.name`).bind(user.id).all();
...
async function createDebt(request,env,user){
  ...
  statements.push(env.DB.prepare(`INSERT INTO debt_agreement_versions(id,debt_id,effective_on,payment_amount_minor,due_date,due_date_secondary,payment_frequency,interest_anchor_date,interest_mode,interest_value,interest_frequency,interest_basis,payment_paused,change_reason,notes,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),debtId,dateOnly(new Date()),payment,due,dueSecondary,arrangement,due,mode,arrangement==='one_time'?0:Number(b.interestValue||0),frequency,clean(b.interestBasis,20)||'original',b.paused?1:0,'created',clean(b.notes,1000),now));
...
async function debtPayment(request,env,user){
  const b=await readJson(request),amount=moneyMinor(b.amount),otherFees=moneyMinor(b.otherFees||0),occurred=validDate(b.date),excessTreatment=['principal','advance'].includes(b.excessTreatment)?b.excessTreatment:'principal',debt=await env.DB.prepare(`SELECT d.*,c.name creditor_name FROM debts d JOIN creditors c ON c.id=d.creditor_id WHERE d.id=? AND d.user_id=? AND d.status IN ('active','paused')`).bind(String(b.debtId||''),user.id).first();
  ...
  const scheduled=Math.min(Number(agreement?.payment_amount_minor||amount),Number(debt.current_balance_minor)),oldCredit=Number(debt.advance_credit_minor||0),creditUsed=Math.min(oldCredit,scheduled),cashNeeded=Math.max(scheduled-creditUsed,0),cashToDue=Math.min(amount,cashNeeded),excess=Math.max(amount-cashToDue,0),advanceAdded=excessTreatment==='advance'?excess:0,principalApplied=Math.min(Number(debt.current_balance_minor),creditUsed+cashToDue+(excessTreatment==='principal'?excess:0)),next=debt.current_balance_minor-principalApplied,newCredit=oldCredit-creditUsed+advanceAdded,now=new Date().toISOString(),status=next===0?'paid':debt.status,ledgerId=crypto.randomUUID(),requestKey=idempotency(request),statements=[
    ...
    env.DB.prepare(`INSERT INTO debt_payment_operations(id,debt_id,expected_balance_minor,payment_amount_minor,ledger_entry_id,created_at,excess_treatment,principal_applied_minor,advance_applied_minor) VALUES(?,?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),debt.id,debt.current_balance_minor,amount,ledgerId,now,excessTreatment,principalApplied,advanceAdded),
    env.DB.prepare('UPDATE debts SET current_balance_minor=?,advance_credit_minor=?,status=?,paid_at=?,updated_at=? WHERE id=? AND user_id=? AND current_balance_minor=?').bind(next,newCredit,status,next===0?occurred:null,now,debt.id,user.id,debt.current_balance_minor),
  ];
  ...
  if(next>0&&agreement&&agreement.payment_frequency!=='one_time'&&Number(agreement.payment_amount_minor)>0&&creditUsed+cashToDue>=scheduled){
    const installment=Number(agreement.payment_amount_minor),cashCycles=Math.max(1,Math.floor((amount+creditUsed)/installment)),dueCycles=debtScheduleCyclesThrough(agreement.due_date,agreement.due_date_secondary,agreement.payment_frequency,occurred),coveredCycles=Math.min(cashCycles,Math.max(1,dueCycles));
    let advanced=advanceDebtSchedule(agreement.due_date,agreement.due_date_secondary,agreement.payment_frequency,coveredCycles);
    if(advanced.dueDate===occurred&&coveredCycles<dueCycles)advanced=advanceDebtSchedule(advanced.dueDate,advanced.dueDateSecondary,agreement.payment_frequency,1);
    nextDueDate=advanced.dueDate;
...
async function updateDebtAgreement(request,env,user){
  const b=await readJson(request),debt=await env.DB.prepare(`SELECT d.*,c.name creditor_name FROM debts d JOIN creditors c ON c.id=d.creditor_id WHERE d.id=? AND d.user_id=? AND d.status IN ('active','paused')`).bind(String(b.debtId||''),user.id).first(),reason=['agreement','negotiated','correction'].includes(b.reason)?b.reason:null,effective=validDate(b.effectiveOn),newBalance=b.currentBalance===''||b.currentBalance==null?null:moneyMinor(b.currentBalance),payment=moneyMinor(b.paymentAmount),due=validDate(b.dueDate),arrangement=paymentFrequency(b.paymentFrequency||'monthly'),dueSecondary=arrangement==='twice_monthly'?validDate(b.dueDateSecondary):null,mode=interestMode(b.interestMode),frequency=interestFrequency(b.interestFrequency);
  if(!debt||!reason||!effective||payment<0||!due||!arrangement||!mode||!frequency||(arrangement==='twice_monthly'&&(!dueSecondary||dueSecondary<=due))||(newBalance!==null&&newBalance<0))return reply({error:'invalid_agreement_update'},400);
  if(reason!=='agreement'&&newBalance===null)return reply({error:'updated_balance_required'},400);
  const previous=await env.DB.prepare(`SELECT interest_basis FROM debt_agreement_versions WHERE debt_id=? ORDER BY created_at DESC,effective_on DESC LIMIT 1`).bind(debt.id).first();
  const requestedBasis=clean(b.interestBasis,20),basis=['original','remaining'].includes(requestedBasis)?requestedBasis:(previous?.interest_basis||'original');
  const now=new Date().toISOString(),versionId=crypto.randomUUID(),paused=Boolean(b.paused),statements=[
    env.DB.prepare(`INSERT INTO debt_agreement_versions(id,debt_id,effective_on,payment_amount_minor,due_date,due_date_secondary,payment_frequency,interest_anchor_date,interest_mode,interest_value,interest_frequency,interest_basis,payment_paused,change_reason,notes,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(versionId,debt.id,effective,payment,due,dueSecondary,arrangement,due,mode,Number(b.interestValue||0),frequency,basis,paused?1:0,reason,clean(b.notes,1000),now)
  ];
  if(newBalance!==null){
...
async function recordRecoveryMilestones(env,userId,achievedOn,now){
  const milestoneTable=await env.DB.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='recovery_progress_milestones'`).first();
...
async function recoverySummary(env,user){
  ...
  const [journey,balance,snapshots,cleared,milestones,journeyChanges,monthlyChanges]=await Promise.all([
    ...
    milestoneTable?env.DB.prepare(`SELECT id,percentage,achieved_on FROM recovery_progress_milestones WHERE user_id=? ORDER BY achieved_on DESC,percentage DESC`).bind(user.id).all():{results:[]},
...
async function exportAccount(env,user){
  const milestoneTable=await env.DB.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='recovery_progress_milestones'`).first(),tables=['allocation_rules','ledger_entries','ledger_reversals','expected_income','living_plans','living_bill_instances','goals','creditors','debts','recovery_journeys','recovery_snapshots',...(milestoneTable?['recovery_progress_milestones']:[])];
...
async function requestAccountDeletion(request,env,user){
  ...
  const now=new Date().toISOString(),jobId=crypto.randomUUID(),anonymous=`deleted-${user.id}@invalid.local`,milestoneTable=await env.DB.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='recovery_progress_milestones'`).first();
  await env.DB.batch([
    ...
    ...(milestoneTable?[env.DB.prepare(`DELETE FROM recovery_progress_milestones WHERE user_id=?`).bind(user.id)]:[]),
...
async function resetFinancialData(request,env,user){
  ...
  const now=new Date().toISOString(),milestoneTable=await env.DB.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='recovery_progress_milestones'`).first();
  await env.DB.batch([
    env.DB.prepare(`DELETE FROM expected_income WHERE user_id=?`).bind(user.id),
    env.DB.prepare(`DELETE FROM living_plans WHERE user_id=?`).bind(user.id),
    env.DB.prepare(`DELETE FROM goals WHERE user_id=?`).bind(user.id),
...
async function runScheduledWork(env,scheduledTime){
  ...
  if(Date.now()-started<7000)results.interest=await postDueInterest(env,now,50);
...
async function purgeExpiredFinancialData(env,userId,now){
  const milestoneTable=await env.DB.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='recovery_progress_milestones'`).first();
  await env.DB.batch([
    env.DB.prepare(`DELETE FROM expected_income WHERE user_id=?`).bind(userId),env.DB.prepare(`DELETE FROM living_plans WHERE user_id=?`).bind(userId),env.DB.prepare(`DELETE FROM goals WHERE user_id=?`).bind(userId),...(milestoneTable?[env.DB.prepare(`DELETE FROM recovery_progress_milestones WHERE user_id=?`).bind(userId)]:[]),env.DB.prepare(`DELETE FROM recovery_snapshots WHERE user_id=?`).bind(userId),env.DB.prepare(`DELETE FROM recovery_journeys WHERE user_id=?`).bind(userId),env.DB.prepare(`DELETE FROM debts WHERE user_id=?`).bind(userId),env.DB.prepare(`DELETE FROM creditors WHERE user_id=?`).bind(userId),env.DB.prepare(`DELETE FROM ledger_reversals WHERE user_id=?`).bind(userId),env.DB.prepare(`DELETE FROM ledger_entries WHERE user_id=?`).bind(userId),env.DB.prepare(`DELETE FROM allocation_rules WHERE user_id=?`).bind(userId),env.DB.prepare(`INSERT INTO admin_audit(id,subject_user_id,action,result,detail_json,created_at) VALUES(?,?,?,?,?,?)`).bind(crypto.randomUUID(),userId,'expired_financial_data_deleted','success',JSON.stringify({retention:'12_months'}),now)
  ]);
}
...
async function postDueInterest(env,now,limit,userId=''){
  const day=dateOnly(new Date(now)),rows=await env.DB.prepare(`SELECT d.id debt_id,d.user_id,d.current_balance_minor,d.journey_start_balance_minor,(SELECT amount_minor FROM ledger_entries WHERE related_type='debt' AND related_id=d.id AND entry_type='new_debt' ORDER BY created_at LIMIT 1) original_balance_minor,(SELECT COUNT(*) FROM debt_payment_operations p WHERE p.debt_id=d.id AND p.principal_applied_minor>0) principal_payment_count,v.id version_id,v.interest_mode,v.interest_value,v.interest_frequency,v.interest_basis,v.effective_on,v.due_date,COALESCE(v.interest_anchor_date,v.due_date) interest_anchor_date FROM debts d JOIN debt_agreement_versions v ON v.id=(SELECT id FROM debt_agreement_versions WHERE debt_id=d.id AND effective_on<=? ORDER BY effective_on DESC,created_at DESC LIMIT 1) WHERE d.status IN ('active','paused') AND v.interest_mode IN ('percentage','fixed') AND (?='' OR d.user_id=?) ORDER BY d.id LIMIT 200`).bind(day,userId,userId).all();let count=0;
  for(const x of rows.results){const posted=await env.DB.prepare(`SELECT cycle_on FROM scheduled_interest_charges WHERE debt_id=? ORDER BY cycle_on`).bind(x.debt_id).all(),cycles=missingInterestCycles(x,posted.results.map(row=>row.cycle_on),day,limit-count);for(const cycle of cycles){const fresh=await env.DB.prepare(`SELECT current_balance_minor FROM debts WHERE id=?`).bind(x.debt_id).first(),useRemaining=x.interest_basis==='remaining'&&Number(x.principal_payment_count)>0,base=useRemaining?Number(fresh.current_balance_minor):Number(x.original_balance_minor||fresh.current_balance_minor),amount=x.interest_mode==='fixed'?moneyMinor(x.interest_value):Math.round(base*Number(x.interest_value)/100);if(amount<=0)continue;const ledgerId=crypto.randomUUID();try{await env.DB.batch([env.DB.prepare(`UPDATE debts SET current_balance_minor=current_balance_minor+?,updated_at=? WHERE id=?`).bind(amount,now,x.debt_id),env.DB.prepare(`INSERT INTO ledger_entries(id,user_id,occurred_on,entry_type,category,amount_minor,related_type,related_id,description,created_at,idempotency_key) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).bind(ledgerId,x.user_id,cycle,'interest','debt_adjustment',amount,'debt',x.debt_id,'Interest added',now,`interest:${x.debt_id}:${cycle}`),env.DB.prepare(`INSERT INTO scheduled_interest_charges(id,user_id,debt_id,agreement_version_id,cycle_on,amount_minor,ledger_entry_id,created_at) VALUES(?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),x.user_id,x.debt_id,x.version_id,cycle,amount,ledgerId,now)]);count++;}catch(error){if(!/UNIQUE|constraint/i.test(String(error.message)))throw error;}if(count>=limit)return count;}}return count;
}

async function reconcileOriginalBalanceInterest(env,userId=''){
  const rows=await env.DB.prepare(`SELECT s.id charge_id,s.amount_minor,s.ledger_entry_id,s.debt_id,d.user_id,
    COALESCE((SELECT amount_minor FROM ledger_entries WHERE related_type='debt' AND related_id=d.id AND entry_type='new_debt' ORDER BY created_at LIMIT 1),d.journey_start_balance_minor) original_balance_minor,
    v.interest_value
    FROM scheduled_interest_charges s
    JOIN debts d ON d.id=s.debt_id
    JOIN debt_agreement_versions v ON v.id=s.agreement_version_id
    WHERE v.interest_mode='percentage'
      AND (v.interest_basis='original' OR NOT EXISTS(SELECT 1 FROM debt_payment_operations p WHERE p.debt_id=d.id AND p.principal_applied_minor>0))
      AND (?='' OR d.user_id=?)
    ORDER BY s.cycle_on LIMIT 1000`).bind(userId,userId).all();
  const fixes=[],debtDeltas=new Map();
  for(const row of rows.results){
    const expected=Math.round(Number(row.original_balance_minor)*Number(row.interest_value)/100),delta=expected-Number(row.amount_minor);
    if(!delta)continue;
    fixes.push(env.DB.prepare(`UPDATE scheduled_interest_charges SET amount_minor=? WHERE id=? AND amount_minor=?`).bind(expected,row.charge_id,row.amount_minor));
    fixes.push(env.DB.prepare(`UPDATE ledger_entries SET amount_minor=? WHERE id=? AND amount_minor=? AND entry_type='interest'`).bind(expected,row.ledger_entry_id,row.amount_minor));
    debtDeltas.set(row.debt_id,(debtDeltas.get(row.debt_id)||0)+delta);
  }
  const now=new Date().toISOString();
  for(const [debtId,delta] of debtDeltas)fixes.push(env.DB.prepare(`UPDATE debts SET current_balance_minor=current_balance_minor+?,updated_at=? WHERE id=?`).bind(delta,now,debtId));
  if(fixes.length)await env.DB.batch(fixes);
  return debtDeltas.size;
}
...
function cookie(value,maxAge){ return `${SESSION_COOKIE}=${value}; Path=/; HttpOnly; Secure; SameSite=None; Max-Age=${maxAge}`; }
...
function debtScheduleCyclesThrough(dueDate,dueDateSecondary,frequency,through){if(!validDate(dueDate)||!validDate(through)||dueDate>through)return 0;let due=dueDate,secondary=dueDateSecondary||null,cycles=0;while(due&&due<=through&&cycles<10000){cycles++;if(frequency==='one_time')break;const advanced=advanceDebtSchedule(due,secondary,frequency,1);due=advanced.dueDate;secondary=advanced.dueDateSecondary;}return cycles;}
...
function isInterestCycleDue(agreement,day){
  const start=agreement.interest_anchor_date||agreement.due_date||agreement.effective_on;if(!validDate(start)||day<start)return false;const elapsed=daysBetween(start,day);
...
function dueInterestCycles(agreement,lastCycle,today,limit=3){
  const result=[],first=agreement.interest_anchor_date||agreement.due_date||agreement.effective_on;if(!validDate(first)||today<first||limit<=0)return result;let next=lastCycle?nextInterestCycle(lastCycle,agreement.interest_frequency,first):first;
  while(next<=today&&result.length<limit){result.push(next);next=nextInterestCycle(next,agreement.interest_frequency,first);}return result;
}
function missingInterestCycles(agreement,postedCycles,today,limit=50){
  const result=[],posted=new Set(postedCycles),first=agreement.interest_anchor_date||agreement.due_date||agreement.effective_on;if(!validDate(first)||today<first||limit<=0)return result;let next=first,scanned=0;
  while(next<=today&&result.length<limit&&scanned<10000){if(!posted.has(next))result.push(next);next=nextInterestCycle(next,agreement.interest_frequency,first);scanned++;}return result;
}
...
export {allocateMinor,calculateRecovery,addMonths,advanceDueDate,daysBetween,isInterestCycleDue,dueInterestCycles,normalizePaymentEvent,normalizePayhipEvent,normalizePayPalEvent,parseProductMap,mappedPlan,verifyPayhipWebhook};
